...
def cv_frame_generator(video, force_rate, frame_load_cap, skip_first_frames,
    ...
    while video_cap.isOpened():
        ...
        frame_tensor = torch.from_numpy(frame).div_(255)
            ...
            while True:
                ...
                if current_offset == bpi:
                    if prev_frame is not None:
                        yield prev_frame
                        pbar.update(1)
                    # Fix: Convert to smaller dtype first to prevent memory leak
                    frame_data = np.frombuffer(current_bytes, dtype=np.dtype(np.uint16).newbyteorder("<"))
                    prev_frame = frame_data.reshape(size[1], size[0], 4).astype(np.float32) / (2**16-1)
                    del frame_data  # Explicit cleanup
                    if not alpha:
...
def load_video(meta_batch=None, unique_id=None, memory_limit_mb=None, vae=None,
    ...
    if vae is not None:
        ...
        # Fix: Process in chunks to prevent memory leaks
        chunks = []
        chunk_size = min(frames_per_batch, 100)  # Process max 100 frames at once
        current_chunk = []
        ...
        for frame in gen:
            ...
            if len(current_chunk) >= chunk_size:
                ...
                chunks.append(torch.from_numpy(chunk_array))
                current_chunk = []
                # Explicit cleanup
                del chunk_array
        ...
        images = torch.cat(chunks, dim=0) if chunks else torch.empty((0, channels, vh, vw))
        ...
        # Fix: Chunked processing for non-VAE path to prevent 4.7GB memory leaks
        chunks = []
        ...
        if chunks:
            ...
            for chunk in chunks:
                del chunk
            del chunks
            gc.collect()  # Force cleanup of all chunks
        else:
            # Create empty tensor with correct shape
            shape = (0, new_height, new_width, 4 if alpha else 3)
