...
def images_generator(directory: str, image_load_cap: int = 0, skip_first_images: int = 0, select_every_nth: int = 1, meta_batch=None, unique_id=None):
    ...
    def load_image(file_path):
        ...
        if i.shape[0] != size[1] or i.shape[1] != size[0]:
            i_tensor = torch.from_numpy(i).movedim(-1, 0).unsqueeze(0)
            i_tensor = common_upscale(i_tensor, size[0], size[1], "lanczos", "center")
            i = i_tensor.squeeze(0).movedim(0, -1).numpy()
        if has_alpha:
            i[:,:,-1] = 1 - i[:,:,-1]
        return i
